...
def test_dotprod_backprop(engine: runtime.Engine) -> None:
    with config.Configuration(engine=engine):
        params = tensors.Tensor([[1, 2, 3], [4, 5, 6], [7, 8, 9]], requires_grad=True)
        a = (params @ [2, 3, 4]).sum()
        a.backward()
        assert params.gradient is not None
        assert params.gradient.symbol.realize().to_python() == [[2, 3, 4]] * 3
...
def test_matrix_mul_backprop(engine: runtime.Engine) -> None:
    with config.Configuration(engine=engine):
        ...
        a.backward()
        assert params.gradient is not None
        assert params.gradient.symbol.realize().to_python() == [[2, 3], [2, 3]]
...
def test_non_square_matrix_mul_backprop(engine: runtime.Engine) -> None:
    ...
    assert np.allclose(grad.symbol.realize().to_python(), [[3, 7, 11]] * 2)
...
def test_mul_scalar_backprop(engine: runtime.Engine) -> None:
    with config.Configuration(engine=engine):
        ...
        a.backward()
        assert params.gradient is not None
        assert params.gradient.symbol.realize().to_python() == [[scalar, scalar], [scalar, scalar]]
...
def test_elementwise_multiplication_backprop(engine: runtime.Engine) -> None:
    with config.Configuration(engine=engine):
        ...
        assert np.allclose(x.gradient.symbol.realize().to_python(), expected_gradient)
...
def test_matrix_addition_backprop(engine: runtime.Engine) -> None:
    with config.Configuration(engine=engine):
        ...
        assert np.allclose(A.gradient.symbol.realize().to_python(), expected_gradient)
        assert np.allclose(B.gradient.symbol.realize().to_python(), expected_gradient)


if __name__ == "__main__":
    test_non_square_matrix_mul_backprop(runtime.NumPyEngine())
